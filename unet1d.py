...
class DownsamplingBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, dim, kernel_size=5):
...
class SelfAttention(nn.Module):
    def __init__(self, vector_size, dim, hidden_dim=8):
        ...
        # self.value = nn.Linear(vector_size, vector_size, bias=False)
        # self.key = nn.Linear(vector_size, vector_size, bias=False)
        # self.query = nn.Linear(vector_size, vector_size, bias=False)
        self.to_qkv = nn.Conv1d(dim, 3*hidden_dim, 1, bias=False)
        self.to_out = nn.Conv1d(hidden_dim, dim, 1)
        self.attention = nn.MultiheadAttention(vector_size, num_heads=4, batch_first=True)
