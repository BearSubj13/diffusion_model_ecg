...
from readecg import ECGdataset
from unet1d import ECGunet

class Ellips(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        t = 2 * (torch.rand(1) - 1)
        x = 0.5 + self.a * torch.cos(2 * torch.pi * t)
        y = -0.5 + self.b * torch.sin(2 * torch.pi * t)
        xy = torch.cat([x,y])
        return xy
...
class GaussianDiffusion:
    def __init__(self, time_steps=1500):
    ...
    def diffuse(self, x0, t, noise=None):
        ...
        if noise is None:
            noise = torch.normal(0, 1, size=[t.shape[0], x0.shape[1]])
        alpha_t = self.alpha_cumprod_sqrt[t]
        alpha_t = alpha_t.repeat(x0.shape[1], 1).transpose(0,1)
        #x0_repeat = x0.repeat(t.shape[0], 1)
        part1 = alpha_t * x0
        one_minus_alpha_t = self.one_minus_alpha_cumprod_sqrt[t]
        one_minus_alpha_t = one_minus_alpha_t.repeat(x0.shape[1], 1).transpose(0,1)
    ...
    def mean_posterior(self, x0, t, xt=None):
        ...
        alpha_cum_prod_sqrt_t_1 = alpha_cum_prod_sqrt_t_1.repeat(x0.shape[1], 1).transpose(0,1)
        alpha_sqrt_t = alpha_sqrt_t.repeat(x0.shape[1], 1).transpose(0,1)
        one_minus_alpha_cumprod_t_1 = one_minus_alpha_cumprod_t_1.repeat(x0.shape[1], 1).transpose(0,1)
        one_minus_alpha_cumprod_t = one_minus_alpha_cumprod_t.repeat(x0.shape[1], 1).transpose(0,1)
        betta_t = self.betta[t].repeat(x0.shape[1], 1).transpose(0,1)
    ...
    def generation_from_net(self, net, number_of_points, dim=256):
        net.eval()
...
def train_epoch(dataloader, net, diffused_model, optimizer, loss_f, batch_size, device):
    ...
    for ecg in dataloader:
        ...
        loss.backward()
        optimizer.step()
    #plt.quiver(xt[:,0].detach().cpu(), xt[:,1].detach().cpu(),  (mu-xt)[:,0].detach().cpu(), (mu-xt)[:,1].detach().cpu())
...
def draw_mu_net(net, t, device):
    net.eval()
    x0 = 0.5
    y0 = -0.5
    borderx = 0.6
    bordery = 1.1
    x = torch.linspace(x0-borderx, x0+borderx, 20)
    y = torch.linspace(y0-bordery, y0+bordery, 20)
    XV, YV = torch.meshgrid(x, y)
    x = XV.reshape(XV.nelement())
    y = YV.reshape(YV.nelement())
    xy_rect = torch.vstack([x, y])
    xy_rect = xy_rect.transpose(1,0)    
    with torch.no_grad():
        xy_rect = xy_rect.to(device)
        t = t * torch.ones(xy_rect.shape[0])
        t = t.to(device)
        mu_estim = net(xy_rect, t)
    mu_estim = mu_estim.cpu()
    return mu_estim, x, y
    

def draw_mu_model(diffused_model, t):
    ellips_dataset = Ellips(10)
    t = torch.LongTensor([t, t])
    for i in range(100):
        x0 = ellips_dataset.__getitem__(0).unsqueeze(0)
        x0 = x0.repeat(2, 1)
        xt, mu = diffused_model.mean_posterior(x0, t)
        if i == 0:
            x = xt[:,0]
            y = xt[:,1]
            mx = mu[:,0]
            ...
            x = torch.cat([x, xt[:,0]])
            y = torch.cat([y, xt[:,1]])
            mx = torch.cat([mx, mu[:,0]])
            my = torch.cat([my, mu[:,1]])
    plt.quiver(x, y, mx - x, my - y)



if __name__ == "__main__":
    dataset_path = "/ayb/vol1/kruzhilov/datasets/ecg/ningbo_64.npy"
    diffused_model = GaussianDiffusion()
    
    # dataset = Ellips(100000)
    dataset = ECGdataset(dataset_path)

    #net = MuNet(256)
    net = ECGunet(kernel_size=5)
    device = "cuda:1"
    net = net.to(device)
    #diffusion_ecg1000_kernel5.pth
    save_weights_path =  "/home/kruzhilov/petct/weights/deleteme.pth"
    ...
    #     #mean_loss = train_epoch(dataloader, net, diffused_model, optimizer, loss_f, batch_size, device)
    ...
    xi = diffused_model.generation_from_net_epsilon(net, 2, 256)
    xi = xi.clamp(-0.4, 1.0)
    plt.plot(xi[0,:].squeeze(0))

    # for i in range(20,40):
    #     ecg = dataset.__getitem__(i).unsqueeze(0)
        # t = torch.LongTensor([5])
        # mu, _ = diffused_model.mean_posterior(ecg, t, ecg+0.01)
        # delta = mu - ecg
        # ecg = ecg.unsqueeze(0).to(device) - 0.2
        # t = t.to(torch.float).to(device)
        # const_line = 0.1*torch.ones(ecg.shape).to(device)    
        # with torch.no_grad():
        #   output = net(ecg + 0.01, t).cpu()
        # output[0,0,-1] = output[0,0,-2]  
    ...
    # ecg = dataset.__getitem__(10)
    # t = torch.FloatTensor([500])
    # tt = t.unsqueeze(0).repeat(1, ecg.shape[0])
    # tt = torch.exp(t/500)
    # gt = tt * torch.sqrt(torch.abs(ecg.flip([-1]) + ecg))
    # plt.plot(gt)
    # ecg = ecg.unsqueeze(0).unsqueeze(0).to(device)
    # t = t.to(device)
    # output = net(ecg, t).detach().cpu()
